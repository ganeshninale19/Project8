...
def update():
    ...
    if(request.method == 'GET'):
        ...
        if(Power == '-1' and Value == '-1'):
            cur.execute("update control set status=%s where appliance=%s", (Status, Appliance))
        elif(Value == '-1' and Power != '-1'):
            cur.execute("update control set status=%s, power=%s where appliance=%s", (Status, Power, Appliance))

        elif(Power == '-1' and Value != '-1'):
            cur.execute("update control set status=%s, value=%s where appliance=%s", (Status, Value, Appliance))
